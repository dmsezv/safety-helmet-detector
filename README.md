...
Проект настроен на **автоматическую загрузку данных**. Если папка с датасетом (`safety-helmet-ds`) отсутствует или пуста, система сама скачает её с Google Drive при первом запуске обучения.
